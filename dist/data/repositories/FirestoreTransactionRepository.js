...
export class FirestoreTransactionRepository {
    ...
    async findById(id) {
        ...
        if (!docSnap.exists())
            return null;
        return TransactionMapper.toDomain(docSnap.data(), docSnap.id);
    }
    async findByAccountId(accountId) {
        if (!db)
            throw new Error('Firestore is not initialized.');
        const collectionRef = collection(db, this.collectionName);
        const q = query(collectionRef, where('accountId', '==', accountId), orderBy('date', 'desc'));
        const querySnapshot = await getDocs(q);
        return querySnapshot.docs.map(docSnap => TransactionMapper.toDomain(docSnap.data(), docSnap.id));
    }
}
